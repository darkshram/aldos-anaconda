...
class Storage(object):
    ...
    def formatByDefault(self, device):
        ...
        formatlist = ['/boot', '/var', '/usr']
        exceptlist = ['/home', '/usr/local', '/opt', '/var/www', '/var/ftp', '/var/spool/mail', '/srv/www', '/srv/ftp']
    ...
    def sanityCheck(self):
        ...
        """
        checkSizes = [('/usr', 5120), ('/var', 5120),
                      ('/home', 5120), ('/boot', 300)]
        warnings = []
        errors = []

        mustbeonlinuxfs = ['/', '/var', '/usr', '/home', '/usr/share', '/usr/lib', '/usr/lib64']
        mustbeonroot = ['/bin','/dev','/sbin','/etc','/lib','/lib64','/root','/run','/tmp','/media','/mnt','lost+found','/proc','/sys']
...
class FSSet(object):
    ...
    def devshm(self):
        if not self._run:
            self._run = NoDevice(format=getFormat("tmpfs",
                                                     device="tmpfs",
                                                     mountpoint="/run"))
        return self._run
    ...
    def devshm(self):
        if not self._tmp:
            self._tmp = NoDevice(format=getFormat("tmpfs",
                                                     device="tmpfs",
                                                     mountpoint="/tmp"))
        return self._tmp
